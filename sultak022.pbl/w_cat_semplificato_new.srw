forward
global type w_cat_semplificato_new from w_semplice_gd
end type
type cb_cat from commandbutton within w_cat_semplificato_new
end type
type cb_dw from commandbutton within w_cat_semplificato_new
end type
type p_1 from picture within w_cat_semplificato_new
end type
type dw_2 from udw_000 within w_cat_semplificato_new
end type
type cb_salva_lista from commandbutton within w_cat_semplificato_new
end type
type cb_st_dw_1 from commandbutton within w_cat_semplificato_new
end type
type cb_scegli from commandbutton within w_cat_semplificato_new
end type
type dw_3 from udw_000 within w_cat_semplificato_new
end type
type cb_1 from commandbutton within w_cat_semplificato_new
end type
end forward
...
global w_cat_semplificato_new w_cat_semplificato_new

type variables
long al_lista_id, al_id_catalogo
string as_path
...
public subroutine wf_salva_lista (string as_codice_lista)
end prototypes
...
public subroutine wf_salva_lista (string as_codice_lista);integer i
...
ds_lista.setitem(1, "codice", as_codice_lista)
...
on w_cat_semplificato_new.create
int iCurrent
call super::create
...
this.Control[iCurrent+6]=this.cb_st_dw_1
this.Control[iCurrent+7]=this.cb_scegli
this.Control[iCurrent+8]=this.dw_3
this.Control[iCurrent+9]=this.cb_1
end on

on w_cat_semplificato_new.destroy
call super::destroy
destroy(this.cb_cat)
destroy(this.cb_dw)
destroy(this.p_1)
...
event resize;constant integer ci_spazio_y=224   //224
...
cb_inserisci.y=newheight - ci_spazio_y
cb_inserisci.x=dw_1.x
cb_cancella.y=newheight - ci_spazio_y
cb_salva.y=newheight - ci_spazio_y
...
cb_salva_lista.y=newheight - ci_spazio_y

cb_primo.y=newheight - (ci_spazio_y - 20)
cb_primo.x=cb_inserisci.x + 918
cb_indietro.y=newheight - (ci_spazio_y - 20)
cb_indietro.x=cb_primo.x + 160
cb_avanti.y=newheight - (ci_spazio_y - 20)
cb_avanti.x=cb_indietro.x + 160
cb_ultimo.y=newheight - (ci_spazio_y - 20)
cb_ultimo.x=cb_avanti.x + 128


cb_ok.y=newheight - ci_spazio_y
cb_ok.x=dw_1.x+dw_1.width - cb_ok.width
cb_annulla.y=newheight - ci_spazio_y
cb_annulla.x=cb_ultimo.x + cb_ultimo.width+50
cb_ricerca.y=newheight - ci_spazio_y
...
type cb_stampa from w_semplice_gd`cb_stampa within w_cat_semplificato_new
end type

type cb_ricerca from w_semplice_gd`cb_ricerca within w_cat_semplificato_new
boolean visible = false
integer x = 46
integer y = 1028
end type

type cb_primo from w_semplice_gd`cb_primo within w_cat_semplificato_new
integer x = 942
integer y = 1716
end type

type cb_ultimo from w_semplice_gd`cb_ultimo within w_cat_semplificato_new
integer x = 1358
integer y = 1716
end type

type cb_indietro from w_semplice_gd`cb_indietro within w_cat_semplificato_new
integer x = 1102
integer y = 1716
end type

type cb_avanti from w_semplice_gd`cb_avanti within w_cat_semplificato_new
integer x = 1230
integer y = 1716
end type

type dw_1 from w_semplice_gd`dw_1 within w_cat_semplificato_new
integer x = 23
integer y = 28
integer width = 3534
integer height = 1108
string dataobject = "d_riga_catalogo"
end type
...
type cb_inserisci from w_semplice_gd`cb_inserisci within w_cat_semplificato_new
integer x = 59
integer y = 1716
end type

type cb_salva from w_semplice_gd`cb_salva within w_cat_semplificato_new
integer x = 343
integer y = 1716
end type

type cb_cancella from w_semplice_gd`cb_cancella within w_cat_semplificato_new
integer x = 631
integer y = 1716
end type

type cb_annulla from w_semplice_gd`cb_annulla within w_cat_semplificato_new
integer x = 1513
integer y = 1716
end type

type cb_ok from w_semplice_gd`cb_ok within w_cat_semplificato_new
integer x = 1806
integer y = 1716
end type
...
choose case  dwo.name
	...
	case "p_catalogo"
		ls_codice_lista=dw_3.getitemstring(1, "catalogo")
		if ls_codice_lista>"" then
			//wf_salva_catalogo(ls_codice_lista)
		else
			messagebox("Attenzione!", "Inserire il codice da assegnare alla lista!")
		end if
