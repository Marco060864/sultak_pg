forward
global type w_cat_rpt from w_stampa
end type
type p_1 from picture within w_cat_rpt
end type
type cb_mail from commandbutton within w_cat_rpt
end type
type cb_2 from commandbutton within w_cat_rpt
end type
type ole_imm from olecustomcontrol within w_cat_rpt
end type
type ole_1 from olecustomcontrol within w_cat_rpt
end type
end forward
...
is_foto.y_1+= 500*(is_foto.n_righe - 1) + 120
...
choose case il_num_col
	case 1 
		if is_foto.dwchild="N" then
			wf_2_foto_dw(DW_1)
		//	wf_imposta_campi(DW_1)
		else
...
public function integer wf_trova_ris (string as_h_v, string as_nome_imm);//
//integer li_res, li_altezza_foto, li_larghezza_foto, li_altezza_foto_p, li_larghezza_foto_p, li_altezza_foto_prova
...
li_FileNum = FileOpen(as_nome_imm, StreamMode!, Read!, LockWrite!, Replace!)
for i= 1 to 2
	if as_h_v='V' then
...
for i=1 to adwc.rowcount()
		li_altezza_foto=0
		li_larghezza_foto=0
		//cambio era al rovescio (quel che è commetato era scommetato e viceversa 03-10-11
		ls_imm= adwc.getitemstring(i, "cat_art_foto")
		//ls_imm= adwc.getitemstring(i, 3)
		ls_cod_art =adwc.getitemstring(i, "art_codice")
		...
		//03-10-11 rotazione imm
		li_ruota=adwc.getitemnumber(i, "cat_ruota")
		...
		if ldc_coef_vert=0 or isnull(ldc_coef_vert) then ldc_coef_vert=1
		if ldc_coef_horz=0 or isnull(ldc_coef_horz) then ldc_coef_horz=1
...
//			li_centra_imm=0
		...
		modstring_line='create line(name=l_'+ls_nome+' moveable=1 resizeable=1 band=detail background.mode="2"'& 
			...
			' visible='+ls_visible+&
			+' x1="21" y1="'+ls_h+'" x2="'+string(il_larghezza_disponibile - 1600)+'" y2="'+ls_h+'" )'
...
for i=1 to adwc.rowcount()
		...
		ls_imm= adwc.getitemstring(i, "art_foto")
...
this.Control[iCurrent+4]=this.ole_imm
this.Control[iCurrent+5]=this.ole_1
end on

on w_cat_rpt.destroy
call super::destroy
destroy(this.p_1)
...
choose case il_num_col
	case 1
		il_larghezza_disponibile=is_foto.w_pagina
		if is_foto.dwchild='S' then
			dw_1.getchild("dw_1", ldwc_col_1)
	...
	case 2
		il_larghezza_disponibile=(is_foto.w_pagina/2) - 60
		dw_1.getchild("dw_1", ldwc_col_1)
		dw_1.getchild("dw_2", ldwc_col_2)
		...
		next 
	case 3
		il_larghezza_disponibile=(is_foto.w_pagina/3) - 120
		dw_1.getchild("dw_1", ldwc_col_1)
		dw_1.getchild("dw_2", ldwc_col_2)
		dw_1.getchild("dw_3", ldwc_col_3)
		...
		next 
	case 4
		il_larghezza_disponibile=(is_foto.w_pagina/4) - 180
		dw_1.getchild("dw_1", ldwc_col_1)
		dw_1.getchild("dw_2", ldwc_col_2)
		dw_1.getchild("dw_3", ldwc_col_3)
		dw_1.getchild("dw_4", ldwc_col_4)
...
type pb_1 from w_stampa`pb_1 within w_cat_rpt
integer x = 2619
end type

type cb_preview from w_stampa`cb_preview within w_cat_rpt
integer x = 2226
end type

type dw_1 from w_stampa`dw_1 within w_cat_rpt
integer width = 3323
integer height = 1644
end type
...
type pb_salva_su_file from w_stampa`pb_salva_su_file within w_cat_rpt
integer x = 2802
end type

type pb_stampa from w_stampa`pb_stampa within w_cat_rpt
integer x = 2423
end type

type sle_pg from w_stampa`sle_pg within w_cat_rpt
integer x = 1582
end type

type st_1 from w_stampa`st_1 within w_cat_rpt
integer x = 1303
end type

type st_2 from w_stampa`st_2 within w_cat_rpt
end type

type sle_copie from w_stampa`sle_copie within w_cat_rpt
end type

type sle_zoom from w_stampa`sle_zoom within w_cat_rpt
integer x = 704
end type

type cb_7 from w_stampa`cb_7 within w_cat_rpt
integer x = 855
end type

type cb_6 from w_stampa`cb_6 within w_cat_rpt
integer x = 590
end type

type cb_pg_dopo from w_stampa`cb_pg_dopo within w_cat_rpt
end type

type cb_pg_prima from w_stampa`cb_pg_prima within w_cat_rpt
end type

type cb_esci from w_stampa`cb_esci within w_cat_rpt
end type
...
boolean focusrectangle = false
string binarykey = "w_cat_rpt.win"
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
