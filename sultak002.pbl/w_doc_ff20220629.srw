forward
global type w_doc_ff20220629 from w_base
end type
type pb_bilancia from picturebutton within w_doc_ff20220629
end type
type dw_cod_barre from udw_000 within w_doc_ff20220629
end type
type cb_nuovo_doc from commandbutton within w_doc_ff20220629
end type
type cb_duplica from uo_commandbutton within w_doc_ff20220629
end type
type cb_esplodi from uo_commandbutton within w_doc_ff20220629
end type
type cb_stampa from uo_commandbutton within w_doc_ff20220629
end type
type cb_cancella from uo_commandbutton within w_doc_ff20220629
end type
type cb_salva from uo_commandbutton within w_doc_ff20220629
end type
type cb_inserisci from uo_commandbutton within w_doc_ff20220629
end type
type cb_annulla from uo_commandbutton within w_doc_ff20220629
end type
type cb_ok from uo_commandbutton within w_doc_ff20220629
end type
type tab_1 from tab within w_doc_ff20220629
end type
...
type cbx_deriva_automatico from checkbox within tabpage_1
end type
...
type dw_11 from udw_001 within tabpage_1
end type
type tabpage_1 from userobject within tab_1
cbx_cc_num_doc cbx_cc_num_doc
cbx_riga_automatica cbx_riga_automatica
cbx_deriva_automatico cbx_deriva_automatico
...
type dw_13 from udw_001 within tabpage_2
end type
...
type internetresult_1 from internetresult within w_doc_ff20220629
end type
end forward
...
global w_doc_ff20220629 w_doc_ff20220629
...
public subroutine wf_gestisci_partita_oro (string as_peso, long al_row)
...
public subroutine wf_gestisci_partita_oro (string as_peso, long al_row);long ll_id_tit, ll_id_caus
string ls_metallo, ls_tipo_partita, ls_scarico, ls_test
date ldt_data_fine, ldt_da_data
s_partita_oro lstr_partita
integer li_ret
decimal ldc_scaricato

select attiva_partite, data_inizio_saldi
into :ls_test, :ldt_da_data
from dba.val_base
;
if ls_test='S' then
	...
	if ls_scarico='S' then
		...
		lstr_partita.dc_finocalo=dec(as_peso)*lstr_partita.dc_coef_calo/1000
		...
		if isnull(ldc_scaricato) then ldc_scaricato=0
		...
		tab_1.tabpage_2.dw_2.post setitem(al_row, "rdoc_peso", ldc_scaricato)
		//end if
...
long ll_righe, ll_id_iva, ll_id_spesa, ldc_importo, ll_riga
integer i
...
//	ls_des=tab_1.tabpage_2.dw_2.getitemstring(al_row, "rdoc_descrizione")
	ls_des="Consulenza Informatica~r~n Operazione effettuata ai sensi dello art.1, commi da 54 a 89 della Legge n.190 del 23 dicembre 2014."+&
	" Prestazione non soggetta a ritenuta di acconto ai sensi dello art.1, comma 67 della Legge n.190 del 23 dicembre 2014"
	tab_1.tabpage_2.dw_2.setitem(al_row, "rdoc_descrizione", ls_des)
end if
...
event editchanged;call super::editchanged;string  ls_num_bol, ls_des, ls_num_doc
datastore ds_righe
...
event clicked;long ll_riga, ll_max_riga, ll_null, ll_id_reg, ll_num_prog, ll_id_doc
integer li_ret, i
setnull(ll_null)
datastore dw_spese
...
if idw_corrente=tab_1.tabpage_2.dw_2 then
	...
	if ll_riga>0 then
		...
		tab_1.tabpage_2.dw_2.setitem(ll_riga+1, "rdoc_numero", ll_max_riga+1)
		wf_calcola_saldo_derivazione(ll_riga+1)
	end if
	return ll_riga+1
elseif  idw_corrente=tab_1.tabpage_1.dw_1 then
	tab_1.tabpage_1.dw_1.setitem(1, "doc_id", ll_null)
...
if ll_riga_da_cancellare>0 then
	idw_corrente.trigger event ue_delete(ll_riga_da_cancellare)	
	ll_riga_da_cancellare=idw_corrente.getrow()
	if ll_riga_da_cancellare<=0 then
		if messagebox("Conferma", "Confermi la cancellazione definitiva del "+idw_corrente.dataobject+"?"&
			,stopsign!, yesno! )=1 then
			tab_1.tabpage_1.dw_1.trigger event ue_update()
		end if
		if idw_corrente=tab_1.tabpage_1.dw_1 then cb_inserisci.triggerevent("clicked")
	end if
end if
end event
...
this.cbx_deriva_automatico=create cbx_deriva_automatico
...
this.Control[]={this.cbx_cc_num_doc,&
this.cbx_riga_automatica,&
this.cbx_deriva_automatico,&
...
destroy(this.cbx_deriva_automatico)
...
long textcolor = 33554432
long backcolor = 67108864
string text = "Deriva automatico"
end type

event clicked;datawindowchild dwc_guida
integer rtncode
long ll_id_guida
date ldt_data

if checked then
	tab_1.tabpage_2.dw_10.visible=true
	if tab_1.tabpage_1.dw_11.rowcount()<=0 then 
		tab_1.tabpage_1.dw_11.insertrow(1)
		rtncode =tab_1.tabpage_1.dw_11.GetChild('guida_id', dwc_guida)
		...
		if ll_id_guida>0 then
			tab_1.tabpage_1.dw_11.setitem(1, "guida_id", ll_id_guida)
		end if
		ldt_data=tab_1.tabpage_1.dw_11.getitemdate(1, "a_data")
		select ese_data_inizio
		into :ldt_data
		from esercizio
		where :ldt_data>=ese_data_inizio
		and :ldt_data<=ese_data_fine
		;
		
		if ldt_data>date('1900-01-01') then
			tab_1.tabpage_1.dw_11.setitem(1, "da_data", ldt_data)
		end if
	end if
	tab_1.tabpage_1.dw_11.visible=true
else
	tab_1.tabpage_2.dw_10.visible=false
	tab_1.tabpage_1.dw_11.visible=false
end if
...
integer rtncode, ll_num_registro
date ld_data_doc, ld_data_prog, ldt_inizio, ldt_fine, ldt_null
datawindowchild ldwc_dest
string ls_tipo_reg, ls_num_doc, ls_tipo, ls_guida_reg_id, ls_prefisso,ls_tipo_doc_ae
long ll_spesa_id
...
integer height = 228
integer taborder = 40
string dataobject = "d_deriva_automatico_ext"
...
this.Control[]={this.cbx_calo_cambia_peso,&
...
integer taborder = 50
boolean bringtotop = true
string dataobject = "ds_doc_righe_da_evadere"
boolean hscrollbar = true
boolean vscrollbar = true
end type

event clicked;call super::clicked;long ll_id_guida, ll_id_titolo, ll_id_metallo, ll_id_soggetto, ll_riga_corrente

if dwo.name='b_retrieve' then
	ll_riga_corrente=tab_1.tabpage_2.dw_2.getrow()
	if ll_riga_corrente>0 then
		ll_id_soggetto=tab_1.tabpage_1.dw_1.getitemnumber(1, "doc_conto_id")
		
			ll_id_guida=tab_1.tabpage_1.dw_11.getitemnumber(1, "guida_id")
			ll_id_titolo= tab_1.tabpage_2.dw_2.getitemnumber(ll_riga_corrente, "tit_id")
			select tit_met_id 
			into :ll_id_metallo
			from dba.titolo
			where tit_id=:ll_id_titolo;
...
integer taborder = 50
boolean bringtotop = true
string dataobject = "d_rdoc_sh_nc_gd"
end type
...
event rowfocuschanged;call super::rowfocuschanged;long ll_art_id
...
event itemchanged;call super::itemchanged;string ls_des, ls_tipo_um, ls_tipo_mov
...
CHOOSE CASE string(dwo.name)
	...
	case "rdoc_peso"
		...
		if ll_art_id>0 then
		  ...
		  wf_gestisci_partita_oro(data, row)
...
event updatestart;call super::updatestart;long ll_find, ll_end
...
type internetresult_1 from internetresult within w_doc_ff20220629 descriptor "pb_nvo" = "true" 
event create ( )
event destroy ( )
end type
