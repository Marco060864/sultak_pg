forward
global type w_ra_padre_figlio_cx from w_base
end type
type cb_ricerca from commandbutton within w_ra_padre_figlio_cx
end type
type dw_1 from udw_001 within w_ra_padre_figlio_cx
end type
type dw_2 from udw_002 within w_ra_padre_figlio_cx
end type
type cb_inserisci from commandbutton within w_ra_padre_figlio_cx
end type
type cb_salva from commandbutton within w_ra_padre_figlio_cx
end type
type cb_cancella from commandbutton within w_ra_padre_figlio_cx
end type
type cb_annulla from commandbutton within w_ra_padre_figlio_cx
end type
type cb_ok from commandbutton within w_ra_padre_figlio_cx
end type
end forward
...
global w_ra_padre_figlio_cx w_ra_padre_figlio_cx
...
on w_ra_padre_figlio_cx.create
int iCurrent
call super::create
...
on w_ra_padre_figlio_cx.destroy
call super::destroy
...
if ll_riga>0 then
	...
	if isnull(ll_id)  then
		if wf_salva_dw(dw_1,dw_2, 1)=1 then
			ll_id=dw_1.getitemnumber(ll_riga, 1)
			if ll_id>0 then
...
event clicked;if i_dw_corrente=1 then
	wf_salva_dw(dw_1, dw_2, 1)
elseif  i_dw_corrente=2 then
	wf_salva_dw(dw_1, dw_2, 2)
end if
end event
